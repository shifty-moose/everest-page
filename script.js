    ...
    // Returns the mountain object from the array
    const returnMountainObject = item => {
        let itemName = item.texture.textureCacheIds[0];
        itemName = itemName.replace('.png', '').replace('Highlight', '').replace('./img/', '');        
        let selectedMountain = listOfMountains.find(mountain => mountain.name === itemName);
    ...
    const createMountainLabel = item => {
        ...
        let mountain = returnMountainObject(item);
    ...
    const onMouseOver = item => {
        ...
        item.texture = PIXI.Texture.from(mountain.highlightTexture);
    };

    // Removes the label on mouse out and changes the texture back to the original
    const onMouseOut = item => {
        let mountain = returnMountainObject(item);
        item.texture = PIXI.Texture.from(mountain.baseTexture);
    ...
    listOfMountains.forEach(element => {
        ...
        mountain.eventMode = 'static';

        mountain.on('pointerover', () => onMouseOver(mountain)); 
